 ...
 */
public class SupplierSQL implements Dataset<String, Subject> {

	@Override
	public void forEach(Consumer<Subject> consumer) {
		String query = "SELECT [code], [name], [address], [phone], [email] FROM [dbo].[Supplier] ORDER BY [name] ASC";
		try {
	...
	public void forEach(BiConsumer<String, Subject> consumer) {
		String query = "SELECT [key], [code], [name], [address], [phone], [email] FROM [dbo].[Supplier] ORDER BY [name] ASC";
		try {
			Connection cn = SQL.getConnection();
